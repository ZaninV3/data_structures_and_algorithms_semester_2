...
private:
	...
	std::vector <TreeNode <T>*> bypass_massive = {};
public:
	...
	// Обход NLR
	void getNLR() {
		// Если обход только начался
		if (bypass_massive.size() == 0) {
			
		}

		// Передаем полученный результат и обнуляем его
		std::vector <TreeNode <T>*> result_massive = bypass_massive;
		bypass_massive.clear();
